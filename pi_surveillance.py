        ...
        if text == "Occupied":
            if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
                ...
                if motionCounter >= conf["min_motion_frames"]:
                    if conf["email_pic"]:
                        #t = TempImage(email_pic_path=conf['email_pic_path'])
                        cv2.imwrite(conf['email_pic_path']+'/'+ts2+'.jpg', frame)

                        logger.info("[UPLOAD] {}".format(ts))
                        
                        need_send_email = True
                        send_email_time = timestamp
                        #path = "{email_pic_path}/{timestamp}.jpg".format(email_pic_path=conf["email_pic_path"], timestamp=ts)
                ...
                if conf["email_video"]:
                    ...
                    if not kcw.recording:
                        ...
                        p = "{}/{}.avi".format(conf['email_video_path'], ts2)
                        kcw.start(p, cv2.VideoWriter_fourcc(*'MJPG'),4)

        else:
            if need_send_email is True and (timestamp - send_email_time).seconds > 10:
                try:
